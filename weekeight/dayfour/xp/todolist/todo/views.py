...
def login(request):
    ...
    if request.method == 'GET':
        ...
        if filled_form.is_valid():
            try: 
                user = User.objects.get(username=filled_form.cleaned_data['username'], password=filled_form.cleaned_data['password'])
            ...
            messages.add_message(request, messages.WARNING, 'Please enter valid information')
            return render(request, 'signin.html', {'form': filled_form})
...
def createTodo(request, userid):
    ...
    if request.method == 'GET':
        ...
        if filled_form.is_valid() and chosen_category.is_valid():
            ...
            category_object = Category.objects.get(id=chosen_category.cleaned_data['category'])
